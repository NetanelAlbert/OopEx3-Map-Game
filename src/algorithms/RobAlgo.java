...
public class RobAlgo {
	...
	private final double[][] dist; // All pairs short distance matrix
	...
	public void move() {
		...
		ETA -= currentEdgeW/robot.getSpeed();
		timeOfMove = System.currentTimeMillis();
		int src = path.get(0).getKey();
		int dest = path.get(1).getKey();
		currentEdgeW = algo.getGraph().getEdge(src, dest).getWeight(); //dist[src][dest];
