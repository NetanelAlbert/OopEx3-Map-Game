...
public class SimpleDB {
	...
	public static void blabla() {
		try {
			...
			String insertQuery = "INSERT INTO Logs (UserID, levelID, moves, time) VALUES(5555, 20, 300, '1999-01-01');";
			//ResultSet resultSet = statement.executeQuery(allCustomersQuery);
			boolean ans = statement.execute(insertQuery);
			System.out.println("insert sucsses? "+ans);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void printLog() {
		try {
			...
			Connection connection = DriverManager.getConnection(jdbcUrl, jdbcUser, jdbcUserPassword);
			Statement statement = connection.createStatement();
			String allCustomersQuery = "SELECT * FROM Logs 	WHERE UserID = 999;";
			ResultSet resultSet = statement.executeQuery(allCustomersQuery);

			while (resultSet.next()) {
				System.out.println("Id: " + resultSet.getInt("UserID") + "," + resultSet.getInt("levelID") + ","
						+ resultSet.getInt("moves") + "," + resultSet.getDate("time")  + "," + resultSet.getInt("score"));
			}
			resultSet.close();
			statement.close();
			connection.close();
